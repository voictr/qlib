...
Safety model (read this before running with --execute):

1. Default is a DRY RUN. Orders are computed and printed, nothing is sent to
   the broker, no flag needed.
2. To actually submit orders you must pass --execute.
3. Credentials come only from the environment (ALPACA_API_KEY / ALPACA_API_SECRET).
4. Trading mode comes only from ALPACA_TRADING_MODE (defaults to "paper").
5. Live trading additionally requires BOTH:
     - the environment variable I_CONFIRM_LIVE_TRADING=yes
     - the CLI flag --i-know-this-trades-real-money
   Missing either one blocks any real order submission in live mode. Paper
   mode has no such requirement, since it never risks real money.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--experiment-name", default="us_lightgbm_topk", help="qlib experiment holding the trained model")
    p.add_argument("--provider-uri", default="~/.qlib/qlib_data/us_data")
    p.add_argument("--region", default="us")
    p.add_argument("--predict-date", default=None, help="Trading day to generate signal for (default: latest)")
    p.add_argument("--topk", type=int, default=30)
    p.add_argument("--n-drop", type=int, default=3)
    p.add_argument("--cash-buffer-pct", type=float, default=0.02)
    p.add_argument("--max-position-pct", type=float, default=0.10)
    p.add_argument("--min-trade-value", type=float, default=5.0)
...
def main() -> int:
    ...
    if args.execute and credentials.trading_mode == "live" and not args.i_know_this_trades_real_money:
        logger.error(
            "Refusing to execute: ALPACA_TRADING_MODE=live requires --i-know-this-trades-real-money "
            "in addition to --execute. This is a deliberate double confirmation for real-money trading."
        )
        return 1

    broker = AlpacaBroker(credentials)
    logger.info(f"Trading mode: {credentials.trading_mode} ({credentials.base_url})")
    ...
    try:
        ...
        logger.error(f"Failed to read account state from Alpaca: {e}")
        return 1
    ...
    if not market_open:
        logger.warning("Market is currently closed. Orders will be submitted as day orders and queued by Alpaca.")

    failures = 0
    for o in orders:
        try:
            result = broker.submit_order(symbol=o.symbol, side=o.side, notional=o.notional)
            logger.info(f"Submitted {o.side} {o.symbol} ${o.notional:,.2f} -> order id {result.get('id')}")
        except AlpacaAPIError as e:
            failures += 1
