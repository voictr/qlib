...
exploits/hacks, stablecoin depegs, exchange delistings, regulatory action,
or a project effectively going dark.
...
an automated veto. This matters more here than in the stock system: the
universe is every USD-quoted Coinbase pair, including thin, easily
manipulated small-cap tokens where a single bad headline can be the whole
story.
...
PROMPT_TEMPLATE = """You are a research assistant for a systematic crypto trading desk. \
A quantitative model has decided to {side} the token trading as {symbol} on Coinbase, based \
purely on historical price/volume features -- it has no awareness of news, on-chain events, \
or announcements.

Search for recent news (last ~2 weeks) about the token behind {symbol} that a systematic \
price-based model would not know about and that might change whether trading it today is a \
good idea: a smart-contract exploit or hack, a stablecoin/asset depeg, an exchange delisting \
(on Coinbase or elsewhere), regulatory or legal action against the project or its team, the \
team/project going dark or a rug-pull allegation, or similarly significant project-specific \
news. Ignore generic market commentary and routine price-target speculation.
