...
`qrun workflow_config_crypto_lightgbm.yaml` (see README.md), which logs a
trained model ("params.pkl") and its dataset config ("dataset") to a
qlib/mlflow experiment. This module reloads the most recently *finished*
recorder from that experiment and re-runs inference for a single, fresh day
instead of the fixed backtest date range the config specifies. Crypto trades
every calendar day (no market holidays), so "predict_date" here is just
today's date rather than a market-calendar trading day.
...
def get_latest_recorder(experiment_name: str) -> Recorder:
    ...
    if not finished:
        raise NoTrainedModelError(
            ...
            f"Run `qrun workflow_config_crypto_lightgbm.yaml` first to train a model."
        )
...
def generate_today_signal(
    ...
    volatility: trailing daily-return std per instrument over the last
        `volatility_lookback_days` days, used to risk-adjust position sizing
        (see portfolio.build_orders). An instrument with too little history
        to compute a std (e.g. newly listed) is simply absent from this
        series rather than given a made-up value.
